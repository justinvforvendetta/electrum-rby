...
from electrum_rby.account import BIP32_Account
from electrum_rby.bitcoin import EncodeBase58Check, public_key_to_bc_address, bc_address_to_hash_160
from electrum_rby.i18n import _
from electrum_rby.plugins import BasePlugin, hook, always_hook, run_hook
from electrum_rby.transaction import deserialize
from electrum_rby.wallet import BIP32_HD_Wallet
from electrum_rby.util import print_error
from electrum_rby.wallet import pw_decode, bip32_private_derivation, bip32_root
...
class TrezorWallet(BIP32_HD_Wallet):
    ...
    def show_address(self, address):
        ...
        try:
            ...
            give_error(e)
        try:
            self.get_client().get_address('RubyCoin', address_n, True)
        except Exception, e:
            give_error(e)
        finally:
    ...
    def sign_message(self, address, message, password):
        ...
        try:
            ...
            give_error(e)
        try:
            msg_sig = self.get_client().sign_message('RubyCoin', address_n, message)
        except Exception, e:
            give_error(e)
        finally:
            self.plugin.handler.stop()
    ...
    def sign_transaction(self, tx, password):
        ...
        try:
            signed_tx = self.get_client().sign_tx('RubyCoin', inputs, outputs)[1]
        except Exception, e:
            give_error(e)
        finally:
            self.plugin.handler.stop()
