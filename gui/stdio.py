...
_ = lambda x:x
#from i18n import _
from electrum_rby.wallet import WalletStorage, Wallet
from electrum_rby.util import format_satoshis, set_verbosity, StoreDict
from electrum_rby.bitcoin import is_valid
from electrum_rby.network import filter_protocol
...
class ElectrumGui:
    ...
    def __init__(self, config, network):
        ...
        if not storage.file_exists:
            print "Wallet not found. try 'electrum-rby create'"
            exit()
    ...
    def do_send(self):
        if not is_valid(self.str_recipient):
            print(_('Invalid RubyCoin address'))
            return
        try:
    ...
    def network_dialog(self):
        print("use 'electrum-rby setconfig server/proxy' to change your network settings")
        return True


    def settings_dialog(self):
        print("use 'electrum-rby setconfig' to change your settings")
        return True
