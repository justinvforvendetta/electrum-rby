...
from electrum_rby import SimpleConfig, Wallet, WalletStorage, format_satoshis
from electrum_rby.bitcoin import is_address
from electrum_rby import util
...
def pay_to(recipient, amount, label):
    ...
    droid.dialogCreateSpinnerProgress("Electrum-RBY", "signing transaction...")
    droid.dialogShow()
...
def receive_loop():
    ...
    while out is None:
        ...
        if not event:
            ...
            amount = modal_input('Amount', 'Amount you want to receive (in RBY). ', format_satoshis(receive_amount) if receive_amount else None, "numberDecimal")
...
class ElectrumGui:
    ...
    def __init__(self, config, _network):
        ...
        if not storage.file_exists:
            ...
            droid.dialogCreateSpinnerProgress("Electrum-RBY", msg)
            droid.dialogShow()
