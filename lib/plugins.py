...
def init_plugins(config, is_local, is_gui):
    ...
    if is_local:
        ...
        electrum_plugins = imp.load_module('electrum_rby_plugins', fp, pathname, description)
        loader = lambda name: imp.load_source('electrum_rby_plugins.' + name, os.path.join(pathname, name + '.py'))
    else:
        electrum_plugins = __import__('electrum_rby_plugins')
        loader = lambda name: __import__('electrum_rby_plugins.' + name, fromlist=['electrum_rby_plugins'])
